...
export default function ChartSales({ locations }: { locations: string[] }) {
    ...
    const sales = formatters[Kpis.Sales](dataTotalSales ? dataTotalSales[0]["sales"] : 0);
    const { data: dataTotalSalesPerHour } = useFetchPipe("total_sales_per_hour_api", {
        locations: locations.length > 0 ? locations : undefined,
    });
    ...
    return (
        <>
            <Metric>{sales}</Metric>
            { dataTotalSalesPerHour && dataTotalSalesPerHour.length > 0 ? <AreaChart
